...
SYSTEM_PROMPT = (
    ...
    "제품 정보를 바탕으로 구매 전환율이 높은 카피를 씁니다.\n"
    "원칙:\n"
    "1) 자연스러운 한국어 — 번역체·과장·클리셰('최고의','완벽한') 금지.\n"
    "2) product_details의 실제 수치·사양을 문장에 구체적으로 녹일 것 "
    "(예: '16GB 메모리로 앱 여러 개를 동시에'). 없는 사실은 지어내지 말 것.\n"
    "3) 기능 나열이 아니라 '구매자가 얻는 이점' 중심.\n"
    "4) 섹션마다 서로 다른 소구점 — headline 중복 금지.\n"
    "5) 특수문자 대시(–, —, ‑)는 쓰지 말고 일반 하이픈(-)만 사용.\n"
    "출력은 반드시 JSON만 사용하세요."
)
...
- 실제 수치·사양을 문장에 녹이고, 막연한 표현('강력한 성능')은 지양.
- 각 섹션 headline은 서로 다른 소구점으로 (반복 금지).
