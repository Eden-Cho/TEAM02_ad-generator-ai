...
> 브랜치 `feature/model-v2-handoff` · **분기 지점(merge-base) `ea3abbb`**
> **이 브랜치는 "운영 완성본"이 아니다.** 최신 모델 구현 + 연결 계약 + 평가 자료를 팀에 넘기는 것이
> 목적이며, 실제 생성은 **기본적으로 차단**돼 있다.
>
> ⚠️ **upstream 진행 상황(2026-07-22 확인):** 팀 main이 `ea3abbb` → **`660bc84`** 로 앞서갔다
> (`chore: update gitignore and fix pipeline observability`). 변경된 팀 파일은
> `.gitignore` · `backend/app/services/pipeline_service.py` · `backend/app/services/scorer.py` 3개다.
> **연결 계약에는 영향이 없다** — 우리가 쓰는 `scorer.score_images(images, req=None) -> dict`
> 시그니처는 그대로다. 이 브랜치는 아직 rebase/merge하지 않았다(보안 조치 대기, 아래 10절 1번).
...
**팀 기존 파일은 한 줄도 바꾸지 않았다.** 분기 지점(`ea3abbb`) 대비 이 브랜치는
**추가 50파일, 수정 0파일**이다(`git diff --name-status ea3abbb HEAD` 가 전부 `A`).
검증 기준은 움직이는 `upstream/main`이 아니라 **분기 지점**이다 — 팀이 main에 새 커밋을
올려도 "우리가 팀 파일을 안 건드렸다"는 사실은 그대로여야 하기 때문이다.
...
1. **🔴 [보안·선행·미완료] `backend/.env`가 저장소에 여전히 추적되고 있다.**
   우리 브랜치가 추가한 것이 아니라 팀 커밋 `a55fcb7`에서 들어왔다.
   - **진행된 것:** 팀 커밋 `660bc84`가 `.gitignore`에 `.env`·`.env.*`·`*.pem`·`*.key` 규칙을 추가했다.
   - **남은 것:** `.gitignore`는 **이미 추적 중인 파일을 추적 해제하지 않는다.**
     `660bc84` 기준으로도 `backend/.env`는 **여전히 저장소에 들어 있다**(추적 상태 확인,
     값은 열람하지 않음). 따라서 **키 폐기·재발급 + `git rm --cached` + 히스토리 정리**가
     아직 필요하다. 조치 주체는 백엔드 담당.
   - **이 브랜치의 대응:** 보안 조치가 끝날 때까지 **push·PR·rebase·merge를 보류**한다.
     (이 브랜치를 올려도 새로 유출되는 것은 없지만, PR이 해당 파일에 이목을 끌 수 있다.)
