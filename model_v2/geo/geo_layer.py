...
def _jsonld_html_safe(obj: dict) -> str:
    """JSON-LD를 <script>에 안전하게 임베드 — script 태그 탈출·XSS 차단.

    <, >, & 와 JS 문자열 종결 문자(U+2028/U+2029)를 유효한 JSON 유니코드 이스케이프로
    바꾼다. JSON 의미(값)는 동일하게 유지된다.
    """
    # allow_nan=False: NaN·Infinity 같은 비표준 JSON 토큰을 출력하지 않는다(유효 JSON-LD 보장).
    s = json.dumps(obj, ensure_ascii=False, allow_nan=False)
    s = s.replace("&", "\\u0026").replace("<", "\\u003c").replace(">", "\\u003e")
    return s.replace(chr(0x2028), "\\u2028").replace(chr(0x2029), "\\u2029")


def _cell(v) -> str:
    """HTML 셀용 안전 문자열 — str/int/float만 표기, 그 외(list/dict/None/bool)는 ''.
    잘못된 값을 repr로 공개하지 않고, html.escape의 AttributeError도 막는다."""
...
def build_geo_html(req: dict, page_copy: dict, specs: dict, faq: list[dict],
    ...
    esc = html.escape
    name = esc(_cell(req.get("product_name")))
    rows = "".join(
        f"<tr><th scope=\"row\">{esc(_cell(k))}</th><td>{esc(_cell(v))}</td></tr>"
        for k, v in (specs or {}).items() if isinstance(specs, dict)
    )
    faqs = "".join(
        f"<dt>{esc(_cell(f.get('q')))}</dt><dd>{esc(_cell(f.get('a')))}</dd>"
        for f in (faq or []) if isinstance(f, dict))
    ld = "\n".join(
        f'<script type="application/ld+json">{_jsonld_html_safe(j)}</script>'
        for j in jsonlds
    )
    ...
    return (
        ...
        f"<h1>{name}</h1>"
        f"<p>{esc(_cell(_intro_text(page_copy)))}</p>"
